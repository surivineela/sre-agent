...
namespace OperationalAgentRuntime.Helpers
{
    public static class HtmlHelpers
    {
        ...
        public static string GenerateHtmlTableForTls(List<TlsStatus> list, Version desiredVersion)
        {
            ...
            html.Append("<tr>");
            html.Append("<th>App Name</th>");
            html.Append("<th>Current Min TLS Version</th>");
            html.Append("<th>Recommended Min TLS Version</th>");
            html.Append("</tr>");

            // Table rows
            foreach (var item in list)
            {
                // Default color is green; if version < desired, color red
                string color = "#28a745"; // green
                try
                {
                    Version current = new Version(item.MinimumTlsVersion);
                ...
                html.Append("<tr>");
                html.AppendFormat("<td>{0}</td>", item.Name);
                html.Append($"<td><span style='color:{color}'>{item.MinimumTlsVersion}</span></td>");
                html.Append($"<td><span style='color:#28a745'>{desiredVersion}</span></td>");
                html.Append("</tr>");
            }
