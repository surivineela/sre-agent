...
{
    private sealed record DiagnoseStatus(
        DateTime StartTime,
        Task Task);

    private static readonly AsyncOperationTracker<string, string, string> _diagnoseTracker = new(
    ...
    [Description("To start diagnose process for single or multiple app services asynchronously.")]
    ...
    private async Task DiagnoseMultipleApps(
        Kernel kernel,
        IReadOnlyList<string> resourceIdList)
    {
        ...
        await Task.WhenAll(resourceIdList
            .Select(resourceId => _diagnoseTracker.GetTask(resourceId) ?? Task.CompletedTask));

        var jsonResult = JsonSerializer.Serialize(resourceIdList.Select(_diagnoseTracker.GetOperationSummary)
        ...
        await ChatHistoryPersistency.ChatHistoryTransition(
            async history =>
            {
                history.AddSystemMessage($"Background diagnosis for multiple apps has finished, please summarize healthy app in concise words and unhealthy app in more details. Here is the result in json format: {jsonResult}");
    ...
    //[KernelFunction("diagnose_appservice")]
    //[Description("To start an app service diagnose or monitor app health process asynchronously.")]
    //public AsyncOperationStartResult<string, string> Diagnose(
    //    Kernel kernel,
    //    [Description("The resource ID of the app service resource.")]
    //    string resourceId)
    //{
    //    return _diagnoseTracker.TryStartOperation(
    //        kernel,
    //        contextMessage: $"Diagnose app: {resourceId}",
    //        resourceId,
    //        parameter: "");
    //}

    [KernelFunction("get_appservice_diagnose_status")]
    [Description("To query an app service's diagnose status to know if it's finished or in progress. Returns null if the operation hasn't started yet.")]
    public AsyncOperationStatusSummary<string, string>? GetDiagnoseStatus(
        [Description("The resource ID of the app service resource.")]
        string resourceId)
    {
        return _diagnoseTracker.GetOperationSummary(
            resourceId);
    }
