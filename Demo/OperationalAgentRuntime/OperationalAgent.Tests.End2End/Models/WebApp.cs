...
namespace E2ETests.Models
{
    internal class WebApp
    {
        ...
        public WebApp(IMessageSink output)
        {
            _output = output;
            _config = Helper.GetConfig();

            ArmClient client = new ArmClient(new DefaultAzureCredential());
            subscription = client.GetSubscriptionResource(new ResourceIdentifier($"/subscriptions/{_config["SubscriptionId"]}"));
        }
        ...
        {
            string skipCleanupSetting = _config["SkipResourceCleanupAfterTestRun"];
            bool skipCleanup = bool.TryParse(skipCleanupSetting, out var result) && result;
        ...
        {
            var webApp = await GetIfExistsAsync(resourceGroup, (rg) => rg.GetWebSiteAsync(Helper.GetWebAppName()));
        ...
        private async Task<WebSiteResource> CreateWebApp(ResourceGroupResource resourceGroup, AppServicePlanResource appServicePlan)
        {
            ...
            var webappoperation = await resourceGroup.GetWebSites().CreateOrUpdateAsync(WaitUntil.Completed, Helper.GetWebAppName(), siteData);
