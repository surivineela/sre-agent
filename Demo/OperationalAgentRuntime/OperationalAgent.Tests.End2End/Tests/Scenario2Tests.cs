...
namespace E2ETests.Tests
{
    ...
    public class Scenario2Tests : IDisposable
    {
        ...
        public Scenario2Tests(TestFixture fixture, ITestOutputHelper testOutputHelper)
        {
            _fixture = fixture;
        ...
        public async Task CanTellHowManyResources()
        {
            ...
            Assert.True(await _fixture.FunctionApp1Process.WaitForOutputAsync("verified my access"));
        ...
        {
            await Helper.SendMessage(_fixture, _output, $"track sub id {_config["SubscriptionId"]}");
            await _fixture.FunctionApp1Process.WaitForOutputAsync(OperationalAgentRuntime.Consts.Verified);

            await Helper.SendMessage(_fixture, _output, $"Can you ensure we're applying best practices to my web apps");
            Assert.True(await _fixture.FunctionApp1Process.WaitForOutputAsync("Click here to approve", 60));
            ...
            Assert.True(await _fixture.FunctionApp1Process.WaitForOutputAsync("App Name", 60));
        ...
        {
            await Helper.SendMessage(_fixture, _output, $"track sub id {_config["SubscriptionId"]}");
            Assert.True(await _fixture.FunctionApp1Process.WaitForOutputAsync(OperationalAgentRuntime.Consts.Verified));
            Assert.True(await _fixture.FunctionApp1Process.WaitForOutputAsync(_config["AppName"]));
        }
