...
export const ThreadsMenu: FC<IThreadsMenuProps> = (
  ...
  const filteredThreads = useMemo(() => {
    ...
    if (threadMode === ThreadMode.incidents) {
      ...
      if (selectedTime) {
        const filterByDays = (time: string) => {
          const days =
            time === SelectedTimes.OneDay
              ? 1
              : time === SelectedTimes.SevenDays
              ? 7
              : 30;
          const now = new Date();
          const cutoff = new Date(now.getTime() - days * 24 * 60 * 60 * 1000);
          return newThreads.filter(
            (item) => new Date(item.modifiedTimestamp) > cutoff
          );
        };
    ...
    if (searchString) {
      return newThreads.filter((thread) =>
        thread.title.toLowerCase().includes(searchString.toLowerCase())
      );
    } else {
      return newThreads;
    }
  ...
  return (
    <div className={ThreadMenuStyles.root}>
      ...
      >
        <Radio
          value={ThreadMode.threads}
          label={SreAgentResources.allThreads}
        />
        <Radio
          value={ThreadMode.incidents}
          label={SreAgentResources.incidents}
        />
      </RadioGroup>
  ...
  ({
    ...
    return (
      <div
        className={
          threadMode === ThreadMode.incidents
            ? ThreadMenuStyles.incidentList
            : ThreadMenuStyles.threadList
        }
        role="tree"
      >
        {threads.map((thread) => {
