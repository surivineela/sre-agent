...
/**
 * Parses a cron expression and converts it to a human-readable format
...
export const parseCronExpression = (cronExpression: string): string => {
    if (!cronExpression || cronExpression === 'N/A') {
    ...
    const minuteIndex = isExtended ? 1 : 0;
    const hourIndex = isExtended ? 2 : 1;
    const dayIndex = isExtended ? 3 : 2;
    const monthIndex = isExtended ? 4 : 3;
    const dayOfWeekIndex = isExtended ? 5 : 4;

    const minute = parts[minuteIndex];
    const hour = parts[hourIndex];
    const day = parts[dayIndex];
    const month = parts[monthIndex];
    const dayOfWeek = parts[dayOfWeekIndex];
    ...
    if (dayOfWeek !== '*' && day === '*') {
        ...
        if (dayNames.length === 1) {
            return `Weekly on ${dayNames[0]} at ${hour.padStart(2, '0')}:${minute.padStart(2, '0')}`;
        } else {
            return `Weekly on ${dayNames.join(', ')} at ${hour.padStart(2, '0')}:${minute.padStart(2, '0')}`;
        }
    }

    return `At ${hour.padStart(2, '0')}:${minute.padStart(2, '0')} on ${day}/${month}`;
};
