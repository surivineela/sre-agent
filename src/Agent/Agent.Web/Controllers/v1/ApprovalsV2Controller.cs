...
#pragma warning restore CUSTOM004
    {
        ...
        if (approvalStatus == ApprovalDecision.Approved)
        {
            ...
            // Approval Message
            var approvalMessage = $"**✅ Operation Approved**\n\n" +
                                     $"- **Operation ID:** {id}\n" +
                                     $"- **User:** {request.User}\n" +
            $"- **Timestamp:** {DateTime.UtcNow}";

            await _agentOutboundCommunicationService.UpdateThreadWithAgentMessageAsync(existingApprovalV2.ThreadId, new Microsoft.Extensions.AI.ChatMessage(ChatRole.Assistant, approvalMessage));
        }
        else if (approvalStatus == ApprovalDecision.Cancelled)
        {
            ...
            var rejectionMessage = $"**❌ Operation Rejected**\n\n" +
                   $"- **Operation ID:** {id}\n" +
                   $"- **User:** {request.User}\n" +
            $"- **Timestamp:** {DateTime.UtcNow}";

            await _agentOutboundCommunicationService.UpdateThreadWithAgentMessageAsync(existingApprovalV2.ThreadId, new Microsoft.Extensions.AI.ChatMessage(ChatRole.Assistant, rejectionMessage));
        }
        else
        {
