...
namespace Agent.Web.Controllers.v1
{
    ...
    public class PsqlExecutionController : ControllerBase
    {
            ...
            [FromBody] ExecutionActionRequest request)
        {
            ...
            string userId = request.User ?? "sreagent-client";
            ...
            Guid messageId = psqlMessages.FirstOrDefault(m => m.PsqlExecution?.Id == executionGuid)?.Id ?? default;

            switch (request.Action.ToLower())
            {
                case "run":
                    PsqlExecution? executionDoc = await _threadRepository.GetPsqlExecutionAsync(threadGuid, executionGuid);
                    ...
                    _ = Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            else
                            {
                                ...
                                string? database = "postgres"; // Default database
