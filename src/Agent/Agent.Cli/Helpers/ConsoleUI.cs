...
public static class ConsoleUI
{
    ...
    {
        WithColor(color, () => {
            Console.WriteLine($"{Chars.Bullet} {message}");
        });
    }
    ...
    public static void WriteBullet(string message, ConsoleColor color = ConsoleColor.Gray, int indent = 2)
    {
        ...
        WithColor(color, () => {
            Console.WriteLine($"{indentStr}{Chars.Bullet} {message}");
        });
    }
    ...
    public static void WriteTreeItem(string message, bool isLast = false, int level = 0, ConsoleColor color = ConsoleColor.Gray)
    {
        ...
        WithColor(color, () => {
            Console.WriteLine($"{prefix}{connector}{line} {message}");
        });
    }
    ...
    {
        if (color.HasValue)
            WithColor(color.Value, () => Console.WriteLine(message));
        else
    ...
    {
        if (color.HasValue)
            WithColor(color.Value, () => Console.Write(message));
        else
            Console.Write(message);
        Console.Out.Flush(); // Ensure the text is displayed before waiting for input
    }
    ...
    {
        // Use ASCII-safe spinner for maximum compatibility
        var frames = new[] { "|", "/", "-", "\\" };
    ...
    /// </summary>
    public static void WriteSection(string title, ConsoleColor color = ConsoleColor.White)
    {
        Console.WriteLine();
        WithColor(color, () => {
    ...
    /// <summary>
    /// Write a group of commands with their descriptions
    /// </summary>
    public static void WriteCommandGroup(string groupName, (string name, string description)[] commands)
    {
        ...
        {
            var (name, description) = commands[i];
            WriteKeyValue(name, description, 15, ConsoleColor.Yellow);

            // Add extra spacing after each command (except the last one)
    ...
    /// <summary>
    /// Captures ConsoleUI output to a string for use in formatted lists.
    /// This helper allows consistent formatting across all list commands.
    /// </summary>
    /// <param name="outputAction">Action that uses ConsoleUI methods to generate output</param>
    /// <returns>String containing the formatted output</returns>
