...
public partial class KubePlugin : IKubePlugin
{
    ...
    public async Task<string> ScaleStatefulSetAsync(string resourceId, string _namespace, string deployment, int replicaCount)
    {
        try
        {
            ...
            if (patchResult != null)
            {
                var scaleDescription = replicaCount > deploy.Spec.Replicas
                    ? "scaled out"
    ...
    public async Task<string> ListCustomResourcesAsync(string resourceId, string _namespace, string apiGroup, string kind)
    {
        try
        {
            ...
            var version = crd.Spec.Versions.FirstOrDefault(v => v.Served && v.Storage)?.Name
    ...
    public async Task<string> GetKubeResourceMetricsRangeAsync(string AKSClusterResourceId, string? _namespace, string kind, string name, string metricsType, string startTime, string endTime)
    {
        ...
        var queries = BuildPromQueries(metricsType, _namespace ?? string.Empty, kind, name, "");
    ...
    public async Task<string> GetKubeResourceEventsAsync(string resourceId, string? _namespace, string apiGroup, string kind, string name)
    {
        try
        {
            ...
            var fieldSelector = !string.IsNullOrEmpty(uid)
            ...
            if (!string.IsNullOrEmpty(_namespace))
            {
                ...
                {
                    fieldSelector += $",involvedObject.namespace={_namespace}"; // UID is primary, namespace refines
                }
                else
                {
                    fieldSelector += $",involvedObject.namespace={_namespace}";
                }
        ...
        string timeRange)
    {
        ...
        try
        {
            ...
            var duration = ParseTimeRangeToDuration(timeRange);

            // Build the PromQL query based on the specified metric type
            var queries = BuildPromQueries(metricType, _namespace, workloadType, workloadName, duration);
            var query = queries.FirstOrDefault() ?? string.Empty;
    ...
    private static string FormatPrometheusResponse(Response? response, string metricType, string workloadType, string workloadName)
    {
        ...
        var capitalizedMetricType = CultureInfo.InvariantCulture.TextInfo.ToTitleCase(metricType.ToLowerInvariant()); // e.g., "Cpu", "Memory"
        ...
        switch (response)
        {
            ...
            case SuccessMatrixResponse successMatrix:
                ...
                foreach (var resultItem in matrixData.Result)
                {
                    ...
                    {
                        var timestamp = metricItem.Item1; // Unix timestamp (seconds)
                        var rawValue = metricItem.Item2;
                        DateTimeOffset dateTime = DateTimeOffset.FromUnixTimeSeconds((long)timestamp);
    ...
    public async Task<string> GetRecentlyUpdatedWorkloadsAsync(string AKSClusterResourceId, string _namespace, int minutesAgo)
    {
        ...
        try
        {
            ...
            foreach (var deployment in deploymentList.Items)
            {
                ...
                {
                    var id = $"{deployment.Metadata.NamespaceProperty}/{deployment.Metadata.Name} (Deployment)";
            ...
            {
                var generationMatch = (sts.Status?.ObservedGeneration ?? -1) >= (sts.Metadata?.Generation ?? 0);
                ...
                if (latestConditionTime.HasValue && latestConditionTime >= cutoffTime)
                {
                    var id = $"{sts?.Metadata?.NamespaceProperty}/{sts?.Metadata?.Name} (StatefulSet)";
                    recentlyUpdated.Add(id);
    ...
    public async Task<string> GetAPIServerStatusAsync(string resourceId, string timeRange = "10m")
    {
        ...
        try
        {
            ...
            {
                new() { Name = "apiserver_cpu_usage_percentage", Unit = "Percent", Aggregation = "Average" },
                new() { Name = "apiserver_current_inflight_requests", Unit = "Count", Aggregation = "Total" },
                new() { Name = "apiserver_memory_usage_percentage", Unit = "Percent", Aggregation = "Average"}
            };
    ...
    public async Task<string> GetEtcdStatusAsync(string resourceId, string timeRange = "10m")
    {
        ...
        try
        {
            ...
            var metrics = new List<Agent.Core.Models.Metric>
            {
                new() { Name = "etcd_cpu_usage_percentage", Unit = "Percent", Aggregation = "Average" },
                new() { Name = "etcd_memory_usage_percentage", Unit = "Percent", Aggregation = "Average" },
                new() { Name = "etcd_database_usage_percentage", Unit = "Percent", Aggregation = "Average"}
            };
        ...
        try
        {
            switch (kind.ToLowerInvariant())
            {
                case "deployment":
                    ...
                    var ownedReplicaSets = replicaSets.Items
                        .Where(rs => rs.Metadata.OwnerReferences != null &&
                                   rs.Metadata.OwnerReferences.Any(or => or.Kind == "Deployment" &&
                                                                      or.Name == name))
                        .OrderByDescending(rs => int.TryParse(rs.Metadata.Annotations?["deployment.kubernetes.io/revision"], out var rev) ? rev : 0)
                        .ToList();
                    ...
                    {
                        var revisionNumber = rs.Metadata.Annotations?["deployment.kubernetes.io/revision"] ?? "unknown";
    ...
    private string analyseDotNetCpuProfilingResult(string podName, string targetContainerName, string _namespace, string aksResourceId, string stdout, string stderr, int exitCode)
    {
        ...
        if (stdout != null && stdout.Contains(noDotNetProcessMarker))
        {
            ...
            var scriptInfoMessage = stdout.Substring(markerIndex).Split('\n')[0].Trim(); // Get the marker line
        ...
        if (!string.IsNullOrEmpty(stdout))
        {
            ...
            {
                if (line.Contains("ANALYSIS START", StringComparison.Ordinal))
                {
                    analysisHeaderFound = true;
                }
        ...
        params string[] args) // This will be ["-c", "actual_script_content_here", "script_name_for_0_arg", "duration_arg"]
    {
        ...
        try
        {
            ...
            var stdoutResult = stdoutMemoryStream != null ? Encoding.UTF8.GetString(stdoutMemoryStream.ToArray()) : string.Empty;
            var stderrResult = stderrMemoryStream != null ? Encoding.UTF8.GetString(stderrMemoryStream.ToArray()) : string.Empty;
            ...
            if (!string.IsNullOrEmpty(stderrResult))
            {
                if (exitCode != 0)
                {
                    _logger?.LogWarning("ExecInPodAsync: Stderr for pod '{PodName}':\n{Stderr}", podName, stderrResult);
                }
                else
                {
                    _logger?.LogDebug("ExecInPodAsync: Stderr (exit code 0) for pod '{PodName}':\n{Stderr}", podName, stderrResult);
                }
            }
    ...
    public static async Task<int> CopyFileToPodAsync(IKubernetes client, string name, string @namespace, string container, string sourceFilePath, string destinationFilePath, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        var handler = new ExecAsyncCallback(async (stdIn, stdOut, stdError) =>
        {
            ...
            try
            {
                ...
                while (true)
                {
                    var numRead = await memoryStream.ReadAsync(localBuffer, 0, localBuffer.Length);
                    if (numRead <= 0)
                    {
                        break;
                    }
            ...
            {
                var error = await streamReader.ReadToEndAsync();
        ...
        string? targetContainerName)
    {
        ...
        var analyzerPath = "/tmp/dotnetanalyzer";
        try
        {
    ...
    private string analyseDotNetMemoryProfilingResult(string podName, string targetContainerName, string _namespace, string aksResourceId, string stdout, string stderr, int exitCode)
    {
        ...
        if (!string.IsNullOrEmpty(stdout))
        {
            var startIndex = stdout.IndexOf(analysisStartMarker);
            var endIndex = stdout.IndexOf(analysisEndMarker, startIndex + analysisStartMarker.Length);
    ...
    private async Task<string> GetPermissionErrorMessageAsync(string resourceId)
    {
        ...
        var identityResourceId = _agentKubeCtlIdentity;
        ...
        var identityName = identityResourceId ?? string.Empty; // fallback to full string for display if specific parts missing
        ...
        var isReadOnlyMode = string.Equals(threadAgentMode, ActionMode.ReadOnly.ToString(), StringComparison.OrdinalIgnoreCase);
