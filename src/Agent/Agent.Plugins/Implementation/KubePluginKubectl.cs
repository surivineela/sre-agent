...
public partial class KubePlugin : IKubePlugin
{
        ...
        string command)
    {
        ...
        {
            return new(new CliExecutionResult { Output = $"[Exception encountered]: Failed to execute command: {ex}", ErrorType = CliErrorType.Other }, null);
        }
    }
        ...
        bool writeCommand)
    {
        ...
        var cmdType = writeCommand ? "write" : "read";
        ...
        for (var i = 1; i < parts.Length; i++)
        {
            ...
            if (parts[i].StartsWith("-"))
            {
                ...
                else
                {
                    ...
                    var isShortFlag = parts[i].StartsWith("-") && !parts[i].StartsWith("--") && parts[i].Length == 2;
                    var isLongFlag = parts[i].StartsWith("--") && !parts[i].Contains('=');
