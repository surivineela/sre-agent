...
namespace Agent.Plugins.Implementation.AzureApplicationInsightsPlugin.Services
{
    ...
    {
        static readonly Regex filterParser = new Regex(@"(?<key>.*)=\s*['""](?<value>.*)['""]", RegexOptions.Compiled | RegexOptions.IgnoreCase);

        public static string GetDistributedTrace(string traceId)
        {
            return $"""
        ...
        public static string GetImpact(string table, string[] filters)
        {
        ...
        public static string ListTraces(string table, string[] filters)
        {
            var parsedFilters = ParseFilters(filters ?? Array.Empty<string>());
            List<string> kqlFilters = KQLQueryBuilder.GetKqlFilters(parsedFilters).ToList();
        ...
        public static (string query, string description) BuildTimeSeriesQuery(AppCorrelateDataSet dataSet, string interval, DateTime start, DateTime end)
        {
            ...
            var parsedFilters = KQLQueryBuilder.ParseFilters(dataSet.Filters ?? Array.Empty<string>());

            var filters = KQLQueryBuilder.GetKqlFilters(parsedFilters);
            var filterDescription = parsedFilters.Length > 0 ? "where " + string.Join(" and ", parsedFilters.Select(kvp => $"{kvp.Key}=\"{kvp.Value}\"")) : "";
            ...
            if (string.IsNullOrEmpty(dataSet.SplitBy))
            {
                return (query: $"""
                    ...
                    """, description: $"{aggregationName} of {dataSet.Table} {filterDescription}");
            }
            ...
            return (query: $"""
                ...
                | extend startTime=start, endTime = end
                | project startTime, endTime, split, Value
                """, description: $"{aggregationName} of {dataSet.Table} {filterDescription} {(!string.IsNullOrEmpty(dataSet.SplitBy) ? $"split by {dataSet.SplitBy}" : "")}");
        }

        public static string[] GetKqlFilters(KeyValuePair<string, string>[] parsedFilters)
        {
        ...
        public static KeyValuePair<string, string>[] ParseFilters(string[] filters)
        {
            return filters
                .Select(f => f.Trim())
        ...
        public static string GetKqlInterval(DateTime start, DateTime end)
        {
