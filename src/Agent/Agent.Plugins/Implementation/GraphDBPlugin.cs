...
.path().by(valueMap('resourceName', 'resourceType'))";
    ...
    public async Task<string> GetGeneralHealthAsync(string resourceName, string resourceType)
    {
        try
        {
            ...
            var baseUrl = $"{_grafanaUrl}/d/{dashboardType}";
            ...
            var healthSummary = await SummarizeDashboardScreenshotAsync(screenshotResponse.Screenshot, dashboardUrl);
...
";
    ...
    public async Task<string> GetApplicationHealthInfoAsync(string resourceId)
    {
        try
        {
            var sanitizedResourceId = SanitizeInputForQuery(resourceId.ToLowerInvariant());

            var resourceID = new ResourceIdentifier(sanitizedResourceId);

            var query = $"g.V().has('resourceId', '{sanitizedResourceId}').has('isDeleted', false).project('properties').by(valueMap()).by(unfold())";
