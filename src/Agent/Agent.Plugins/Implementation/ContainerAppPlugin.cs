...
public class ContainerAppPlugin : IContainerAppPlugin
{
    ...
    private static readonly JsonSerializerOptions _containerAppSerializationOptions = new()
    {
        WriteIndented = true,
        Converters =
        {
    ...
    public async Task<RevisionInfo?> GetLatestRevisionAsync(string resourceId)
    {
        ...
        try
        {
            ...
            var revisionName = latestRevisionName;
            ...
            {
                var trafficWeight = latestRevision?.Data?.TrafficWeight ?? 0;
                var isActive = trafficWeight > 0;
    ...
    public async Task<ContainerAppHealthValidationResult> ValidateContainerAppHealth(string resourceId)
    {
        ...
        try
        {
            ...
            var canScaleToZero = latestRevision.Data.Template?.Scale?.MinReplicas == 0;
            result.Details["CanScaleToZero"] = canScaleToZero.ToString();

            // Count ready replicas
            var readyReplicas = replicas.Count(r => r?.Properties?.RunningState?.Equals("Running", StringComparison.OrdinalIgnoreCase) == true);
            ...
            var hasErrors = await ContainsErrorsInLogs(recentLogs);
            ...
            {
                var fqdn = ingress.Fqdn;
                result.Details["Hostname"] = fqdn;

                var endpointReachable = await IsEndpointReachable(fqdn);
            ...
            var hasValidReplicas = (canScaleToZero && replicas.Count >= 0) || (!canScaleToZero && readyReplicas > 0);    // Otherwise need running replicas
            ...
            if (ingress != null &&
                ingress.External == true)
            {
                isHealthy = isHealthy && bool.TryParse(result.Details["EndpointReachable"], out var endpointReachable) && endpointReachable;
            }
    ...
    private async Task<ImageUpdateResult> UpdateContainerAppImage(string resourceId, string newImageReference)
    {
        ...
        try
        {
            ...
            var originalImage = updateData.Template.Containers[0].Image ?? "unknown";
    ...
    private async Task<RollbackResult> RollbackContainerApp(string resourceId)
    {
        try
        {
            ...
            foreach (var revision in revisions)
            {
                ...
                var isActive = (revision.Data.TrafficWeight ?? 0) > 0;

                // Check if this revision was healthy
                var isHealthy = revision.Data.ProvisioningState == ContainerAppRevisionProvisioningState.Provisioned;
                var healthReason = isHealthy ? "Provisioned state is good" : $"Provisioning state is {revision.Data.ProvisioningState}";
                ...
                if (isHealthy)
                {
                    ...
                    var canScaleToZero = revisionDetails.Value.Data.Template?.Scale?.MinReplicas == 0;
            ...
            var targetSelectionReason = "Found healthy and previously active revision";
        ...
        try
        {
            ...
            if (response.StatusCode == HttpStatusCode.TooManyRequests)
            {
                if (response.Headers.Contains("X-RateLimit-Remaining"))
                {
                    ...
                    if (int.TryParse(rateLimitRemaining, out var remaining) && remaining == 0)
                    {
    ...
    private (string Repository, string Tag) ExtractDockerHubRepositoryAndTag(string imageReference)
    {
        ...
        var tag = match.Groups["tag"].Success ? match.Groups["tag"].Value : "latest"; // Default tag is 'latest'
    ...
    private async Task<bool> VerifyOtherRegistry(string imageReference, string resourceId)
    {
        try
        {
            ...
            var hasRegistryCredentials = false;
    ...
    public async Task<string> RollbackToLastRevision(string resourceId)
    {
        try
        {
            ...
            {
                var message = $"No revisions found for Container App {resourceId}";
            ...
            {
                var message = $"Not enough revisions found for Container App {resourceId} to perform rollback";
                _logger.LogInternalWarning(message);
                return message;
            }
        ...
        Response<ContainerAppAuthToken> streamToken)
    {
        ...
        try
        {
            return await GetLogsAsync(eventsStreamUrl.ToString(), streamToken, StreamEndpointType.EventStream);
        }
        catch (Exception e)
        {
        ...
        string revisionName)
    {
        ...
        var logs = await replicas
            ...
            .SelectMany(c => c)
            .Select(c => GetLogsAsync(c.LogStreamEndpoint, streamToken, StreamEndpointType.LogStream))
    ...
    private async Task<string> GetContainerAppImageReference(ResourceIdentifier resourceId)
    {
        ...
        if (!string.IsNullOrEmpty(latestRevisionName))
        {
            var revisionResourceId = $"{resourceId}/revisions/{latestRevisionName}";
            try
            {
    ...
    private static async Task SendResize(WebSocket socket, int x, int y, CancellationToken token)
    {
        ...
        bytes.Add(4); //resize
        var message = Encoding.UTF8.GetBytes(FormattableString.Invariant($"{{{{\"Width\": {x}, \"Height\": {y}}}}}"));
    ...
    private async Task<string> InvokeExecCommand(string resourceId, string command)
    {
        try
        {
            ...
            var subscriptionId = resourceIdentifer?.SubscriptionId ?? string.Empty;
            var armClient = await _armClientFactory.GetArmOperationClient();
            ...
            var execEndPoint = firstReplica?.Value?.Data?.Containers?.First().ExecEndpoint ?? string.Empty;
            ...
            var token = await _armHelper.GetProxyApiTokenAsync(subscriptionId, resourceIdentifer?.ResourceGroupName ?? string.Empty, containerApp.Value.Data.Name);
            ...
            var completionMarker = "COMPLETED ANALYSIS";
            ...
            var listeningTask = Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        var isCompleted = await Read(webSocket, CancellationToken.None, completionMarker, resultBuilder);
                        if (isCompleted)
                        {
                            var setResult = completionSource.TrySetResult(true);
                            break;
                        }
                    }
                }
            ...
            var pattern = @"STARTED ANALYSIS\s*(.*?)\s*COMPLETED ANALYSIS";
            Match match = Regex.Match(result, pattern, RegexOptions.Singleline);

            if (match.Success)
            {
                var analysisResult = match.Groups[1].Value.Trim();
...
{logsJson}";
    ...
    public async Task<List<DateTimeOffset>> GetDeploymentTimes(string resourceId)
    {
        ...
        var containerAppName = resourceIdentifier.Name;
        ...
        {
            if (!l.TryGetValue("operationName", out var operationName))
            {
                return false;
            }

            if (!l.TryGetValue("authorizationScope", out var authorizationScope))
            {
                return false;
            }

            if (!l.TryGetValue("status", out var status))
            {
                return false;
            }
