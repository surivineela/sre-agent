...
{
    public static global::Prometheus.Protobuf.WriteRequest ToWriteRequest(this global::Prometheus.Counter counter, double val, params string[] labelValues)
    {
        var labelNames = counter.LabelNames;

        if (labelNames.Length != labelValues.Length)
        {
            throw new ArgumentException($"Label names and values count mismatch. Names: {counter.LabelNames}, Values: {labelValues}");
        }

        global::Prometheus.Protobuf.TimeSeries timeSeries = new()
        {
            Labels = { new global::Prometheus.Protobuf.Label { Name = "__name__", Value = counter.Name } }
        };
