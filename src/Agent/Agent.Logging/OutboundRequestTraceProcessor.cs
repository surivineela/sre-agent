...
public class OutboundRequestTraceProcessor : BaseProcessor<Activity>
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="OutboundRequestTraceProcessor"/> class with the specified logger
    /// and multiple exporters. This processor filters for outbound ARM API requests.
    /// </summary>
    ...
    public OutboundRequestTraceProcessor(
        ILogger logger,
        ...
        int maxExportBatchSize = 512)
    {
        ...
        // Filter for ARM API requests by checking server.address
        _filter = activity =>
        {
            var serverAddress = activity?.Tags.FirstOrDefault(t => t.Key == "server.address").Value;
            return serverAddress is string address &&
                   address.Equals("management.azure.com", StringComparison.OrdinalIgnoreCase);
        };
    }
