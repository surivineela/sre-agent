...
public class KustoClient
{
    ...
    public KustoClient(ILogger<KustoClient> logger, KustoSettings kustoSettings)
    {
        ...
        _logger.LogInformation($"Authentication type: {_kustoAuthSettings.AuthenticationType}");
    }
    ...
    public async Task<IDataReader> PerformQueryWithParametersAsync(string clusterUri, string database, string query, Dictionary<string, object> parameters)
    {
        ...
        _logger.LogInformation($"Executing query: {query}, request Id: {properties.ClientRequestId} with parameters");
    ...
    private ICslQueryProvider GetClient(string clusterUri)
    {
        ...
        {
            KustoAuthenticationType.ManagedIdentity => Kusto.Data.Net.Client.KustoClientFactory.CreateCslQueryProvider(new KustoConnectionStringBuilder(clusterUri).WithAadSystemManagedIdentity()),
            KustoAuthenticationType.UAMI => Kusto.Data.Net.Client.KustoClientFactory.CreateCslQueryProvider(new KustoConnectionStringBuilder(clusterUri).WithAadUserManagedIdentity(_kustoAuthSettings.ManagedIdentityClientId)),
            KustoAuthenticationType.App => Kusto.Data.Net.Client.KustoClientFactory.CreateCslQueryProvider(new KustoConnectionStringBuilder(clusterUri).WithAadAzureTokenCredentialsAuthentication(GetClientCertificateCredentials())),
            _ => Kusto.Data.Net.Client.KustoClientFactory.CreateCslQueryProvider(new KustoConnectionStringBuilder(clusterUri).WithAadUserPromptAuthentication())
        };
    }
