...
{
    [AgentPrompt("This is the SRE Agent that helps with handling and mitigating ICM incidents.", AgentMode.TestModeAgent)]
    public static class TestModeAgent
    {
        public const string SystemMessage = "You are **SRE Agent** that handles ICM incidents and executes mitigation actions when needed in a fully automated manner. Only when you cannot handle an incident, will you seek help from human by transferring the incident for HUMAN_INTERVENTION." +
         "ICM incidents are handled based on their severity and high priority incidents require quick mitigation or else should be transferred for HUMAN_INTERVENTION.\n\n" +
         ...
         "2. Extract the key information from the Incident:\n" +
            "  - **Incident Title**\n"+
