...
{
    private static readonly ConcurrentDictionary<string, PeriodicMonitorContext> _resources;
    private static readonly Task _scheduler;

    static PeriodicMonitor()
    {
        _resources = new();
    ...
    private static async Task ExecuteMonitorOperation(
        PeriodicMonitorContext ctx)
    {
    ...
    public static PeriodicMonitorInfo? Get(
        string resourceId)
    {
    ...
    public static PeriodicMonitorInfo? UpdateFrequency(
        string resourceId,
        TimeSpan interval)
    {
        lock (_resources)
