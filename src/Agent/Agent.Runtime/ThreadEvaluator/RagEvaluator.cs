...
{
    Task<IReadOnlyList<RagEvaluationResult>> EvaluateRagPerformanceAsync(
        ThreadModel thread,
        CancellationToken cancellationToken = default);
}

/// <summary>
/// Evaluates RAG (Retrieval-Augmented Generation) performance in threads by analyzing SearchMemory tool calls
/// and measuring the quality of retrieved information using Microsoft.Extensions.AI.Evaluation.Quality.RetrievalEvaluator
///
/// This evaluator:
/// 1. Scans thread chat history for "SearchMemory" tool calls
/// 2. Extracts the search parameters (resourceId, symptoms) and retrieval results
/// 3. Uses Microsoft.Extensions.AI.Evaluation.Quality.RetrievalEvaluator to assess retrieval quality
/// 4. Returns evaluation metrics including relevance and precision scores
///
/// Usage:
/// var ragEvaluator = new RagEvaluator(logger, threadRepository, chatClient);
/// var results = await ragEvaluator.EvaluateRagPerformanceAsync(thread, cancellationToken);
/// </summary>
public class RagEvaluator : IRagEvaluator
{
    ...
    /// <summary>
    /// Evaluates RAG performance for threads that contain SearchMemory tool calls
    /// </summary>
    /// <param name="thread">The thread to evaluate</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>List of RAG evaluation results</returns>
    public virtual async Task<IReadOnlyList<RagEvaluationResult>> EvaluateRagPerformanceAsync(
        ThreadModel thread,
        CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogInternalInformation($"Starting RAG evaluation for thread {thread.Id}: {thread.Title}");
            ...
            foreach (var agentContext in agentContexts)
            {
                try
                {
                    ...
                    var ragResults = await EvaluateRagInChatMessages(thread, agentContext, chatMessages, cancellationToken);
                    results.AddRange(ragResults);
                }
                catch (Exception ex)
                {
                    _logger.LogInternalWarning(ex, $"Error evaluating RAG for agent context {agentContext.Id} in thread {thread.Id}");
                }
            }

            _logger.LogInternalInformation($"Completed RAG evaluation for thread {thread.Id}. Found {results.Count} SearchMemory tool calls to evaluate.");
            return results;
        }
        catch (Exception ex)
        {
            _logger.LogInternalError(ex, $"Error during RAG evaluation for thread {thread.Id}: {ex.Message}");
            return new List<RagEvaluationResult>();
        }
    }

    /// <summary>
    /// Evaluates RAG performance in chat messages by finding SearchMemory tool calls
    /// </summary>
    private async Task<List<RagEvaluationResult>> EvaluateRagInChatMessages(
        ThreadModel thread,
    ...
    {
        var results = new List<RagEvaluationResult>();
        var functionCalls = new Dictionary<string, SearchMemoryCall>();

        // Find all SearchMemory function calls and their results
        foreach (var message in chatMessages)
        {
            ...
            foreach (var call in functionCallContents)
            {
                if (call.Name == "SearchMemory")
                {
                    ...
                    {
                        functionCalls[call.CallId] = searchMemoryCall;
                    }
                }
            }

            // Look for function results
            var functionResultContents = message.Contents.OfType<FunctionResultContent>().ToList();
            foreach (var result in functionResultContents)
            {
                if (functionCalls.TryGetValue(result.CallId, out var searchMemoryCall))
                {
                    try
                    {
                        var evaluationResult = await EvaluateSearchMemoryResult(
                            thread, agentContext, searchMemoryCall, result, cancellationToken);
                    ...
                    {
                        _logger.LogInternalWarning(ex, $"Error evaluating SearchMemory result for call {result.CallId} in thread {thread.Id}");
                    }
                }
            }
        }
    ...
    /// <summary>
    /// Extracts SearchMemory call information from a FunctionCallContent
    /// </summary>
    private SearchMemoryCall? ExtractSearchMemoryCall(FunctionCallContent call)
    {
        try
        {
        ...
        {
            _logger.LogInternalWarning(ex, $"Error extracting SearchMemory call arguments for call {call.CallId}");
            return null;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogInternalError(ex, $"Error during RetrievalEvaluator evaluation for SearchMemory call {call.CallId}");
    ...
    private async Task<RetrievalEvaluationResult> EvaluateRetrievalScore(string query, string retrievalResult)
    {
        ...
        const string RetrievalEvaluationPrompt =
        """
        # Definition
        ...
        **Examples:**
        ...
        ## [Retrieval Score: 2] (Partially Relevant Context, Poor Ranking, External Knowledge Bias)
        ...
        **Examples:**
        ...
        ## [Retrieval Score: 3] (Relevant Context Ranked Bottom)
        ...
        **Examples:**
        ...
        ## [Retrieval Score: 4] (Relevant Context Ranked Middle, No External Knowledge Bias and Factual Accuracy Ignored)
        ...
        **Examples:**
        ...
        ## [Retrieval Score: 5] (Highly Relevant, Well Ranked, No Bias Introduced)
        ...
        **Examples:**
        ...
        List<ChatMessage> evaluationInstructions = [
            ...
            new(ChatRole.System, RetrievalEvaluationPrompt),
    ...
    /// <summary>
    /// Builds a search query from SearchMemory call parameters
    /// </summary>
    private static string BuildQueryFromSearchMemoryCall(SearchMemoryCall call)
    {
        var queryBuilder = new StringBuilder();

        if (!string.IsNullOrEmpty(call.ResourceId))
        {
...
/// <summary>
/// Represents a SearchMemory function call
/// </summary>
...
/// <summary>
/// Represents the result of evaluating RAG performance for a SearchMemory call
/// </summary>
