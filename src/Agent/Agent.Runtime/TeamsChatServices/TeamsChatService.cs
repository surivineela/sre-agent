...
public class TeamsBot : TeamsActivityHandler
{
    ...
    string prompt = "You're assisting an SRE agent. The SRE agent is fetching info to answer the user's question. Provide brief responses (under 20 words) to reassure the user that the agent is working on it, avoiding impatience. Your response can be funny if users are not handling serious issues. Do not say the agent is slow, use phrases like 'fetching information' or 'analyzing your input.' For simple greetings or general questions needing no extra info or long tasks, just reply single world 'SKIP' to avoid duplicate response with SRE agent.";
