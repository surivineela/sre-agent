...
public static class HostApplicationBuilderExtensions
{
    ...
    {
        var envPrefix = builder.Configuration.GetValue<string>("AppSettings:EnvPrefix");
        var ManagedIdentityClientId = builder.Configuration.GetValue<string>("AppSettings:ManagedIdentityClientId");
        ...
        var endpoint = $"https://{envPrefix}-appconfig.azconfig.io";
...
#pragma warning disable CUSTOM003 // App config is only used in local development
        var cred = new DefaultAzureCredential(credOptions); // CodeQL [SM05137] This is not used in production code and only used in local development.
        ...
        where TAppSettings : AppSettings
    {
        ...
        builder.Services.AddSingleton(sp =>
        {
            ...
            if (!MiniValidator.TryValidate(settings, out var validationErrors))
            {
                ...
                foreach ((var member, var memberErrors) in validationErrors)
                {
...
Otherwise, there may be required settings which are not auto-populated by the private deployment script."
        ...
        where TAppSettings : AppSettings
    {
        ...
        sc.AddSingleton(sp => sp.GetRequiredService<TAppSettings>().Core.Azure.Adc);
    }
