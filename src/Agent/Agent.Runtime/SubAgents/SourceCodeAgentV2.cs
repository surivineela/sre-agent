...
2: **Wait**: In this step, you are waiting for the user to respond with repo urls for EACH container app that you listed in the table in step 1. This repo url will look like https://github.com/..... Once you get one per app listed in step 1, move on to step 3.
...
";
        ...
        protected override async Task<IList<AIChatMessage>> GetStartingMessagesAsync()
        {
            ...
            if (_appsWithoutSourceCodeNodes != null &&  _appsWithoutSourceCodeNodes.Any())
            {
                var existingAppsDetails = string.Join(Environment.NewLine,
                    _appsWithoutSourceCodeNodes.Select(x => $"{x.ResourceId} currently does not have a source code node."));
                ...
                foreach (var app in _appsWithoutSourceCodeNodes)
                {
