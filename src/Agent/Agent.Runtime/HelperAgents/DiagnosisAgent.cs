    ...
    toolsRepository)
{
    ...
    private string GetInvestigationPrompt(string resourceId, string issueDescription, string customInstructions)
    {
        return $"""
            ...
            After every 1 or 2 tool calls during your investigation, add a user-visible summary of the steps you performed using the AddNewSummary tool. Examples:
            The memory usage on this application is below 5% on average
            Logs indicate there is a missing environment variable <variable name>
