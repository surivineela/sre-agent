...
/// <summary>
/// Tests for invalid parameter validation in 'srectl agent apply' command.
/// Only tests invalid parameter combinations and error messages.
/// Business logic errors (agent not found, invalid YAML, etc.) are tested in ApplyCommandTests.
/// </summary>
...
public class ApplyCommandInvalidParameterTests : AgentCommandTestBase
{
    ...
    public async Task AgentApply_MissingName_ReturnsError()
    {
        // Act - missing --name option
        var result = await Runner.RunAsync("agent", "apply");
    ...
    [Trait("Command", "Apply")]
    [Trait("Type", "Validation")]
    public async Task AgentApply_InvalidOption_ReturnsError()
    {
        // Act - use an invalid/unknown option
        var result = await Runner.RunAsync("agent", "apply", "--name", "some-agent", "--invalid-option", "value");
    ...
    [Trait("Command", "Apply")]
    [Trait("Type", "Validation")]
    public async Task AgentApply_NameTooLong_ReturnsError()
    {
        // Act - name exceeds 128 characters
        var longName = new string('a', 129);
        var result = await Runner.RunAsync("agent", "apply", "--name", longName);
    ...
    [Trait("Command", "Apply")]
    [Trait("Type", "Validation")]
    public async Task AgentApply_NameWithInvalidCharacters_ReturnsError()
    {
        // Act - name contains invalid characters
        var result = await Runner.RunAsync("agent", "apply", "--name", "agent@name!");
    ...
    [Trait("Command", "Apply")]
    [Trait("Type", "Validation")]
    public async Task AgentApply_EmptyName_ReturnsError()
    {
        // Act - name is empty string
        var result = await Runner.RunAsync("agent", "apply", "--name", "");
    ...
    [Trait("Command", "Apply")]
    [Trait("Type", "Validation")]
    public async Task AgentApply_NameWithSpaces_ReturnsError()
    {
