...
import { isValid, newCustomGuid, newGuid, newShortGuid, newTinyGuid } from '../Guid';
...
describe('Guid utilities', () => {
    ...
    it('newShortGuid returns 8-4 hex pattern', () => {
        const g = newShortGuid();
        const pat = /^[0-9a-f]{8}-[0-9a-f]{4}$/i;
        expect(pat.test(g)).toBe(true);
        expect(isValid(g)).toBe(false); // not a full UUID
    });
    ...
    it('newCustomGuid respects length; 0 => empty string', () => {
        expect(newCustomGuid(0)).toBe('');
        const g10 = newCustomGuid(10);
        expect(g10).toMatch(/^[0-9a-f]{10}$/i);
    });

    it('isValid detects correct UUID strings', () => {
        expect(isValid('not-a-guid')).toBe(false);
        expect(isValid('00000000-0000-4000-8000-000000000000')).toBe(true);
    });
});
