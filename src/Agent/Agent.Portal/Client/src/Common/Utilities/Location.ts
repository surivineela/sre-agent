...
export const getCanonicalLocation = (userFriendlyLocation?: string): string => {
    return userFriendlyLocation ? userFriendlyLocation.replace(/\s+/g, '').replace('(', '').replace(')', '').toLowerCase() : '';
};

export const isSameLocation = (locationA?: string, locationB?: string): boolean => {
    const canonicalLocationA = getCanonicalLocation(locationA);
    const canonicalLocationB = getCanonicalLocation(locationB);

    return canonicalLocationA.toLowerCase() === canonicalLocationB.toLowerCase();
};

export const getUserFriendlyLocation = (location: string): string => {
...
export const failoverPairedLocations: Record<string, string[]> = {
    // Black forest
...
export const getResourceLocation = async (
    ...
    if (selectedLocation && subscriptionId && resourceProvider && resourceType) {
        const supportedLocations = await LocationClient.getInstance(telemetrySource).getLocationsFromArmManifest(
            subscriptionId,
            resourceProvider,
            resourceType,
            telemetrySource
        );
        return supportedLocations.find(location => isSameLocation(location, selectedLocation)) || supportedLocations?.[0] || '';
    }
    return '';
};
