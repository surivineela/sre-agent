 ...
 * No ARM token stuff here as it won't work CORP user -> AME resource
 */
export const useExternalAgentView = (agentUrl: string, sreDeepLink?: string) => {
    const intl = useIntl();
    const { user } = useAuth();
    const { logEvent } = useTelemetry(TelemetrySource.ExternalAgentIFrameView, undefined);
    ...
    const authTokenManager = useAuthTokenManager({
        telemetrySource: TelemetrySource.ExternalAgentIFrameView,
        resourceId: undefined,
        postMessage,
        initialTokenTypes: ['sreAgent'],
    });
