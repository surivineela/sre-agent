        ...
        {isExampleExpanded && (
          <div className='auth-token-example-content'>
            <div className='auth-token-example-description'>
              Add this configuration to your MCP client (e.g., VS Code) to connect to SRE Agent:
            </div>
...
function authTokenCode(authToken: string) {
  return `SREAGENT_EXTENSION_TOKEN=${authToken}`;
}
...
function exampleConfig(authToken: string) {
  ...
  "mcpServers": {
    "sreagent": {
      "command": "npx",
      "args": ["@sreagent/mcp@latest", "--extension"],
      "env": {
        "SREAGENT_EXTENSION_TOKEN":
          "${authToken}"
      }
    }
  }
