  ...
  useEffect(() => {
    const runAsync = async () => {
      ...
      try {
        ...
        setStatus({
          type: 'connecting',
          message: `SRE Agent from "${info}" is trying to connect to your browser. Do you want to continue?`
        });
      } catch (e) {
...
const VersionMismatchError: React.FC<{ extensionVersion: string }> = ({ extensionVersion }) => {
  return (
    <div>
      SRE Agent requires a newer extension version (current version: {extensionVersion}).{' '}
      Please update the SRE Agent Browser Extension to the latest version.
    </div>
  );
};
