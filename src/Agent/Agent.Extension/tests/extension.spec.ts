...
test(`navigate with extension`, async ({ browserWithExtension, startClient, server }) => {
  ...
  const confirmationPagePromise = browserContext.waitForEvent('page', page => {
    return page.url().startsWith('chrome-extension://pgdbefmidcjmokjknhaicodhnpbjdbjc/connect.html');
  });
...
test(`extension not installed timeout`, async ({ browserWithExtension, startClient, server, useShortConnectionTimeout }) => {
  ...
  expect(await client.callTool({
    ...
    result: expect.stringContaining('Extension connection timeout. Make sure the "SRE Agent Browser Extension" is installed.'),
    isError: true,
  });
...
test(`extension needs update`, async ({ browserWithExtension, startClient, server, useShortConnectionTimeout, overrideProtocolVersion }) => {
  ...
  await expect(confirmationPage.locator('.status-banner')).toContainText(`SRE Agent requires a newer extension version`);
...
test(`custom executablePath`, async ({ startClient, server, useShortConnectionTimeout }) => {
  ...
  expect(await fs.promises.readFile(test.info().outputPath('output.txt'), 'utf8')).toContain('Custom exec args: chrome-extension://pgdbefmidcjmokjknhaicodhnpbjdbjc/connect.html?');
});
...
test(`bypass connection dialog with token`, async ({ browserWithExtension, startClient, server }) => {
  ...
  const page = await browserContext.newPage();
  await page.goto('chrome-extension://pgdbefmidcjmokjknhaicodhnpbjdbjc/status.html');
