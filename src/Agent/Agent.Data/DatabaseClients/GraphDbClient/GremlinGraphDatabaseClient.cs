...
public class GremlinGraphDatabaseClient : IGraphDatabaseClient
{
    ...
    private CancellationTokenSource _tokenExpiryCts = new();
            ...
            .WaitAndRetryAsync(100,
                sleepDurationProvider: (int retryAttempt, Exception ex, Context context) =>
                {
                    ...
                    if (respEx.StatusAttributes != null &&
                        ...
                        value != null &&
                        int.TryParse(value.ToString(), out var retryMs))
                    {
        ...
        return await _retryPolicy.ExecuteAsync(async () =>
        {
            ...
            else
            {
                ...
                var msgBuilder = Gremlin.Net.Driver.Messages.RequestMessage.Build(Tokens.OpsEval)
                    .AddArgument(Tokens.ArgsAliases, new Dictionary<string, string> { { "g", GraphName } })
    ...
    public async Task<string> SoftDeleteConnectedRepositoryByResourceId(string resourceId)
    {
        try
        {
            ...
            var updateTs = DateTime.UtcNow.Ticks;
            ...
            {
                var message = $"Soft delete connected repository for resource {resourceId} did not find any matching repository.";
            ...
            {
                var message = $"Soft deleted connected repository for resource {resourceId} successfully.";
                return message;
            }
        }
