...
public class CosmosDbThreadTeamsMappingRepository : IThreadTeamsMappingRepository
{
    ...
    public async Task<IEnumerable<ThreadTeamsMapping>> ListActiveConversationsAsync()
    {
        try
        {
            ...
            _logger.LogDebug("Found {0} threads with unposted messages, need to confirm with threadcontext to check outbound configuration", threadsWithUnpostedMessages.Count);
