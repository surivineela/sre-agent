    ...
    public class APICenterCrawler : GenericArmResourceCrawler
    {
        ...
        private async Task CreateEdgesToApiManagementInstances(APICenterNode apiCenterNode)
        {
            if (apiCenterNode.ApiConnectionInfo != null && apiCenterNode.ApiConnectionInfo.Any())
            {
                ...
                {
                    string apimSourceId = group.Key!;
                    _logger.LogInternalInformation($"Creating edge from API Center to APIM: {apimSourceId}");

                    string apimNodeIdQuery = apimSourceId.ToLower().Replace("/", "_");
                    string query = $@"
                        g.V()
                    ...
                    if (result != null && result.Any())
                    {
                        ...
                        foreach (var connection in group)
                        {
                            ...
                            string? apicApiTitle = connection.Value.Title;
                            ...
                            {
                                string apiName = matchingApimApi.Value.Key;
        ...
        private async Task ProcessApisByWorkspace(APICenterNode apiCenterNode)
        {
            ...
            {
                string workspace = workspaceGroup.Key!;
        ...
        {
            string getApicApiSourceByWorkspaceUrl = $"{ApimConstants.ManagementAzureBaseUrl}{apiCenterNode.ResourceId}/workspaces/{workspace}/apiSources?api-version={ApimConstants.ApicApiVersion}";
            string apiSourcesJson = await GetArmResourceByUrl(getApicApiSourceByWorkspaceUrl);
        ...
        {
            string getApicApiByWorkspaceUrl = $"{ApimConstants.ManagementAzureBaseUrl}{apiCenterNode.ResourceId}/workspaces/{workspace}/apis?api-version={ApimConstants.ApicApiVersion}";
            string apisJson = await GetArmResourceByUrl(getApicApiByWorkspaceUrl);
        ...
        private void UpdateConnectionPropertiesFromApiSourceElement(JsonElement apiSourceElement, IEnumerable<KeyValuePair<string, ApiConnectionEntity>> workspaceConnections)
        {
            ...
            {
                string apiSourceId = idElement.GetString() ?? string.Empty;
        ...
        private void UpdateConnectionPropertiesFromApiElement(JsonElement apiElement, IEnumerable<KeyValuePair<string, ApiConnectionEntity>> workspaceConnections)
        {
            ...
            {
                string apiId = idElement.GetString() ?? string.Empty;

                // Find matching connections in ApiConnectionInfo by comparing resource IDs
                foreach (var connection in workspaceConnections)
                {
                    string identifier = connection.Key;
                    ApiConnectionEntity entity = connection.Value;
                    ...
                    if (match.Success)
                    {
                        string normalizedApiId = match.Value;
