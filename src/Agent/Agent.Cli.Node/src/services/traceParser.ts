/**
 * Trace Parser - Converts ThreadEventLog events into ISpan tree
 * Ported from Agent.Web client's traceParser.ts
 */

import type { ISpan, ThreadEventLog } from '../types/trace';
...
export interface GenerateSpansResult {
  spans: ISpan[];
...
/**
 * Generate ISpan tree from ThreadEventLog events
 */
export function generateSpans(events: ThreadEventLog[]): GenerateSpansResult {
  const spans: ISpan[] = [];
  ...
  for (let index = 0; index < events.length; index++) {
    ...
    // AgentResponse - thinking steps or final response
    ...
    if (event.eventName === 'ModelGeneration' && event.eventType === 'ModelGenerationEnd') {
      ...
      existingModelGenerationSpan.usage_info = {
        ...
        modelThinking: event.modelThinking,
        reasoning,
        response,
      };
      delete unclosedModelGenerationSpanIds[event.agentName];
    ...
    if (event.eventName === 'AgentToolExecution' && event.eventType === 'ToolStart' && event.toolName !== 'HandoffBack') {
      ...
      if (existingAgentSpanId === undefined) {
        parseErrors.push(
          `[ToolStart] ${event.timeStamp.toUTCString()} ToolStart found for tool ${event.toolName} on ${agentType} ${event.agentName} but there is no open span for this agent.`
        );
    ...
    if (event.eventName === 'AgentToolExecution' && event.eventType === 'ToolEnd' && event.toolName !== 'HandoffBack') {
      ...
      if (existingAgentSpanId === undefined) {
        parseErrors.push(
          `[ToolEnd] ${event.timeStamp.toUTCString()} ToolEnd found for tool ${event.toolName} on ${agentType} ${event.agentName} but there is no open span for this agent.`
        );
      }
