---
name: fix_npm_build
description: Use when the user needs to fix `npm run dev` in Agent.Web\Client folder, especially for rollup msvc dependency or SWC native binding errors.
---

# Fix npm Build Issues

Help fix `npm run dev` in Agent.Web\Client folder when it complains about missing native dependencies like rollup msvc or SWC bindings.
