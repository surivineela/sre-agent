...
```powershell
# Download and run the Universal Package installer
.\scripts\install_exe.ps1

# Or specify a version
.\scripts\install_exe.ps1 -PackageVersion "1.0.0"
...
```powershell
# Download and run the NuGet package installer
...
2. **Tool Not Found:** Verify the tool is applied to the server with `srectl list tools`
3. **Query Errors:** Test your KQL queries in Azure Data Explorer first
4. **Agent Not Responding:** Check agent validation and ensure all referenced tools exist
